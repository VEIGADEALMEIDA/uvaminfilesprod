...
/*********************************************************************************************************************************
		...
		Description: JS file base (REDILUMNOLIB)
		Version: 2.0 / 2014-10
...
//				help_str += '<li class="ico-tutorial"><a href="#tutorial" class="lightbox" title="Tutorial">Tutorial de navegação</a></li>';
				help_str += '<li class="ico-tutorial"><a href="http://ead.uva.br/hotsite/view/2/" class="winpopup" data-width="960" data-height="520" title="Tutorial">Tutorial de navegação</a></li>';
				help_str += '</ul>';
			...
			var pre_loader = '<div id="preloader"><div id="status"><img src="img/common/preloader.gif" alt="" /></div></div>';
/*
			var tutorial_str ='<div id="tutorial" class="lightbox-main mfp-hide">';
				tutorial_str +='<section>';
				tutorial_str +='<h4>Tutorial de Navegação</h4>';
				tutorial_str +='<article class="c12 f-left"><p class="txt-center">Para melhor experiência no seu estudo, apresentamos a seguir recursos de navegação utilizados em nossas aulas.</p></article>';
				tutorial_str +='<figure class="c12 f-left txt-center">';
				tutorial_str +='<img src="img/tutorial/step-1.png" alt="" />';
				tutorial_str +='</figure>';
				tutorial_str +='<figure class="c12 f-left txt-center">';
				tutorial_str +='<img src="img/tutorial/step-2.png" alt="" />';
				tutorial_str +='</figure>';
				tutorial_str +='</section>';
				tutorial_str +='</div>';
*/
			$('body').prepend(pre_loader);
			$('body').prepend(help_str);
/*
			$('body').prepend(tutorial_str);
*/
			...
			$('#help ul li a.change-size').on("click", function (e) {
				...
				$(this).parent().hide();
				$('body main').animate({'font-size': elm_size + 'em'}, 400);
			});
			/* contraste */
	...
	_fixHeight: function (group){
		try{
			var total_fixheight = $('*[class*="fix-height-"]').length
			for(var i=1;i<=total_fixheight;i++){
				if($('.fix-height-'+i).length > 0){
					ilumno._equalHeight($('.fix-height-'+i));
				}else{
					break;
				}
			}
	...
	_fixHeaderHeight: function (group){
		try{
			setTimeout(function() {
				$('main').css({'margin-top': $('header').outerHeight()});
				if($('.slide-intro').length > 0)
					$('.slide-intro').css('background-position-y',$('header').outerHeight());
			}, 150);
		}catch(e){
			ilumno._showMessage(e);
		}
...
/*************************************************************************************************************************************************************************************************************************************************************************/
	...
	function slideShow(i,d,p){
		...
		if(slideshow_nav[i]){
 		...
 		if(slideshow_autoplay[i] == true){

			if((parseInt(slideshow_repeat[i]) == 0 ) && (parseInt(slideshow_repeat_count[i]) < parseInt(slideshow[i].children('figure').length))){
				slideshow_timer[i] = setInterval(function() { slideShow(i); }, slideshow_time[i]);
			}else if(parseInt(slideshow_repeat_count[i]) < parseInt(slideshow_repeat[i])){
				slideshow_timer[i] = setInterval(function() { slideShow(i); }, slideshow_time[i]);
			}
...
/*************************************************************************************************************************************************************************************************************************************************************************/
	...
	/* fix height of equal class itens 
	var total_fixheight = $('*[class*="fix-height-"]').length
	for(var i=1;i<=total_fixheight;i++){
		if($('.fix-height-'+i).length > 0){
			ilumno._equalHeight($('.fix-height-'+i));
		...
		$('.slideshow').each(function(i){
			...
			$(this).children('figure').each(function(z){
				...
				if(slide_intro){
					...
					var txt_caption = $(this).children('figcaption').html();
					$(this).children('figcaption').html('<div class="fix-valign"><div>' + txt_caption + '</div></div>')
					$(this).children('figcaption').each(function(figcap){
						$(this).children('div.fix-valign').children('div').css('width',$(this).children('div.fix-valign').width());
					});
	...
	/*********************************************************************************************************************************
			...
			Description: open an popup window

	*********************************************************************************************************************************/
	$('.winpopup').click(function(e) {
		e.preventDefault();
...
function listMultipleRetry(elm){
	$('#' + elm + ' li a').removeClass();
	$('#' + elm + ' div.msg-valid').removeClass('error');
	$('#' + elm + ' div.msg-valid').html('');
}

$(document).ready(function() {
	$('.list-multiple ol li a').on("click", function () {
		var elm = $(this);
		var parent_id = elm.parent().parent().parent().attr('id');
		elm.parent().parent().find('li a').removeClass();
		if(elm.data('correct')){
			elm.removeClass('check');
			elm.addClass('correct');
			elm.parent().parent().parent().find('div.msg-valid').html(elm.parent().parent().parent().find('input.msg-success').val());
			elm.parent().parent().parent().find('div.msg-valid').removeClass('error');
			elm.parent().parent().parent().find('div.msg-valid').addClass('correct');
		}else{
			elm.removeClass('check');
			elm.addClass('error');
			elm.parent().parent().parent().find('div.msg-valid').html('Resposta incorreta, <a href="javascript:listMultipleRetry(\''+ parent_id +'\')" class="btn-retry">tente novamente</a>');
			elm.parent().parent().parent().find('div.msg-valid').removeClass('correct');
			elm.parent().parent().parent().find('div.msg-valid').addClass('error');
		}
		return false;
	});
});
...
$(document).ready(function() {
	...
	if($('.list-relationship p.bt-area-small').css('display') == 'block'){
		$('.list-relationship p.bt-area-small button').on("click", function () {
			$(this).parents('.list-relationship').find("ul li select").each(function(){
				var correct_val = $(this).data('vc').toString();
				correct_val = correct_val.toUpperCase();
				var actual_val = $.trim($(this).val()).toString();
				actual_val = actual_val.toUpperCase();
				//console.log(correct_val + " | " + actual_val);
		...
		$(".list-relationship ol li span").draggable({helper: 'clone'});
		$(".list-relationship ul li input").droppable({
			accept: ".list-relationship ol li span",
			drop: function(ev, ui) {
				$(this).insertAtCaret(ui.draggable.html());
				$(this).removeClass('error');
			}
		});
		$('.list-relationship p.bt-area button').on("click", function () {
			var isFormValid = true;
			$(this).parents('.list-relationship').find("ul li input").each(function(){
				//.parents('.stats').find('.label > .goal')
				var correct_val = $(this).data('vc').toString();
				correct_val = correct_val.toUpperCase();
				var actual_val = $.trim($(this).val()).toString();
				actual_val = actual_val.toUpperCase();
			...
			//console.log('teste-1');
			if($(this).parents('.list-relationship').find("input.msg-success").length > 0){
				//console.log('teste-2');
				//console.log(isFormValid);
				if(isFormValid){
					//console.log('teste-3');
					$(this).parents('.list-relationship').find('div.msg-valid').show().html($(this).parents('.list-relationship').find('input.msg-success').val());
				}else{
					$(this).parents('.list-relationship').find('div.msg-valid').show().html('');
					$(this).parents('.list-relationship').find('div.msg-valid').hide();
				}
			}
	...
	$('.tab-group').each(function(i){
		...
		//console.log(Math.ceil((12/total_col)));


		//var class_col = 'c'+Math.floor((12/total_col));
		var class_col = (100/total_col);

		//var width_col = parseInt($(this).width())/total_col;
		//console.log(width_col);
		var max_height = 150;
		var initial_item = 1;
		//console.log(total_col);
		//$(this).children('nav').children('a').addClass( class_col +' f-left');
		$(this).children('nav').children('a').addClass('f-left')
		$(this).children('nav').children('a').css('width', class_col+'%');
		
		$(this).children('nav').children('a:first-child').addClass('current');
		$(this).children('div.content').each(function(z){
		...
		$(this).children('div.content').children('div:first-child').addClass('current');
		$(this).children('li').children('div.content').css({
			height: max_height,
			display: 'none'
		});

		$(this).children('nav').each(function(i){
			$(this).children('a').on('click', function(e){
				e.preventDefault();
				$(this).parent().children('a').removeClass('current');
				$(this).addClass('current');
				$(this).parent().parent().children('div.content').children('div').removeClass('current');
...
/*	(function () {
		var $frame  = $('#crazy');
		var $slidee = $frame.children('.timeline-x-ct').eq(0);
...
*/
	...
	/***************************** background graph *****************************/
	$('.bg-graph').each(function(i){

		//var the_image = new Image();
		//var img_bg = $(this).children('img').attr('src');
		//the_image.src = img_bg;
		//img_bg_height = the_image.height;
		...
		$(this).children('ul').children('li').each(function(z){
			var elm_posX = $(this).data('x');
			var elm_posY = $(this).data('y');
			var elm_width = $(this).data('width');
			//console.log(elm_posX + " | " + elm_posY);
	...
	$('.set-bg-color').each(function(i){
		$(this).css('background', $(this).data('bg-color'));
	});
	$('.set-txt-color').each(function(i){
		$(this).css('color', $(this).data('txt-color'));
	});
...
$('.flip-area').each(function(i){
	...
	//console.log(className_parent[0]);
	if (className_parent){
	    elm_c = className_parent[0];
	    elm_c = parseInt(elm_c.split('c')[1]);
	    //elm_c = Math.floor(elm_c / total_col);
	    elm_c = Math.floor(12 / total_col);
	}
	//console.log(elm_c);
	...
	var max_height = 0;
	var initial_item = 1;
	$(this).children('li').addClass( 'c'+elm_c +' f-left');
...
$('img.img-gif').each(function(i){


	var $a_link = $("<a>", {class: "load-gif"/*, style: "max-width: " + $(this).outerWidth() +"px ; max-height: "+$(this).outerHeight()+"px"*/});
	$a_link.click(function(e){
		e.preventDefault();
		var elm_data = $(this).data('jpg-image');
		var elm_href = $(this).attr('href');
		//console.log(elm_data);
		//console.log(elm_href);
		if($(this).hasClass('gif-on')){
			$(this).removeClass('gif-on');
			$(this).children('img').attr('src', elm_data);
		}else{
			$(this).addClass('gif-on');
			$(this).children('img').attr('src', elm_href);
		}
	});
	...
	var new_file_name = file_name.substr(file_name_index);
	var new_file_name_final = new_file_name.split(".")[0];
	var file_path = file_name.split(new_file_name)[0];

	//console.log(new_file_name_final);
	//console.log(file_path);

	$(this).parent().attr('href',file_name);
	$(this).attr('src',file_path+new_file_name_final+".jpg");
	$(this).parent().attr('data-jpg-image', file_path+new_file_name_final+".jpg" );
...
//$('.img-gif').on('click', function(e){
//	e.preventDefault();
	//$(this).parent().addClass('gif-on');
...
/*********************************************************************************************************************************
		...
		Description: show/hide destination item 
...
/* click and show elements */
$('.click-show').on("click", function (e) {
	var elm = $(this);
	var hidden_item = $(elm.attr('href'));
	var toggle = Boolean(elm.data('toggle'));
	var slide = Boolean(elm.data('slide'));
	var group = elm.data('group');
	var slide_pos_final = elm.offset().top - $('header').height();
	e.preventDefault();
	if(hidden_item.length > 0){
		if(group != '' && group != null){
			$('.'+group).hide();
			$(this).parent().children('.click-show').removeClass('current');
			$(this).addClass('current');
		}
		if(!hidden_item.is(':visible')){
			if(ilumno._checkie7())
				hidden_item.show();
			else
				hidden_item.slideDown();

			if((slide != false && !slide) || slide == true){
				$('body').animate({
					scrollTop: slide_pos_final
				}, 800);
			}
		}else if(hidden_item.is(':visible') && toggle){
			hidden_item.slideUp();
		}
	}else{
		//item doesn't exist
...
$('.anchor').on('click', function(e){
	...
	var elm_parent = $(this).parent().parent().parent();
	if(elm_parent.hasClass('menu')){
		elm_parent.find('li').removeClass('current');
...
topMenuHeight = topMenu.outerHeight()+$('header').outerHeight()+100,
...
$(window).scroll(function(){
	if($('nav.menu ul li a.anchor').length > 0){
		var fromTop = $(this).scrollTop()+topMenuHeight+110;
		...
		if (lastId !== id) {
			...
			menuItems.parent().end().filter("[href=#"+lastId+"]").parent().addClass("current");
		}
	}
});
