...
audiojs.events.ready(function(){var as=audiojs.createAll();});$('nav.menu ul li a').on('click',function(e){var vlr_href=$(this).attr("href");history.pushState('','',href);e.preventDefault();});});$(window).resize(function(){ilumno._chkMenuWidth();ilumno._fixHeaderHeight();if($('.slideshow').length>0)
...
return false;});}
$('.tab-group').each(function(i){var total_col=parseInt($(this).children('nav').children('a').length);var class_col=(100/total_col);var max_height=150;var initial_item=1;$(this).children('nav').children('a').addClass('f-left')
$(this).children('nav').children('a').css('width',class_col+'%');$(this).children('nav').children('a:first-child').addClass('current');$(this).children('div.content').each(function(z){if(initial_item>total_col){$(this).addClass('clear');initial_item=1;}
